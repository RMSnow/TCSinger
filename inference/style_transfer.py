...
def process_align(ph_durs, mel, item, hop_size, audio_sample_rate):
    ...
    for i_ph in range(len(ph_durs)):
        ...
        end_frame = int(
            (startTime + ph_durs[i_ph]) * audio_sample_rate / hop_size + 0.5
        )
...
class StyleTransfer(BaseTTSInfer):
    ...
    def preprocess_input(self, inp):
        ...
        item = {
            ...
            "ph": ph,
            "ph_token": ph_token,
            "ph_token_gen": ph_token_gen,
            "ph_token_prompt": ph_token_prompt,
            "mel": mel,
            "note": note,
            "note_dur": note_dur,
            "note_type": note_type,
        ...
        item["mel2ph_prompt"] = process_align(
            inp["ph_durs"],
            mel,
            item,
            hop_size=hparams["hop_size"],
    ...
    def input_to_batch(self, item):
        ...
        txt_tokens_prompt = torch.LongTensor(item["ph_token_prompt"])[None, :].to(
            self.device
        )
        ...
        sent_txt_lengths = torch.LongTensor(item["sent_txt_lengths"])[None, :].to(
            self.device
        )
        note = torch.LongTensor(item["note"])[None, :].to(self.device)
        note_dur = torch.FloatTensor(item["note_dur"])[None, :].to(self.device)
        note_type = torch.LongTensor(
            item["note_type"][: hparams["max_input_tokens"]]
        ).to(self.device)
    ...
    def example_run(cls):
        ...
        if "ref_name" in inp:
            items_list = json.load(
                open(f"{hparams['processed_data_dir']}/metadata.json")
            )
