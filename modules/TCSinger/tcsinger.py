...
import torch
from modules.TCSinger.diff.gaussian_multinomial_diffusion import (
...
class NoteEncoder(nn.Module):
    ...
    def forward(self, note_tokens, note_durs, note_types):
        # print("note_tokens", note_tokens.shape, note_tokens)
        # print("note_durs", note_durs.shape, note_durs)
        # print("note_types", note_types.shape, note_types)
...
class TCSinger(FastSpeech):
    ...
    def __init__(self, dict_size, hparams, out_dims=None):
        ...
        if hparams["f0_gen"] == "gmdiff":
            ...
            if hparams["param_"] == "x0":
                self.f0_gen = GaussianMultinomialDiffusionx0(
                    num_classes=2,
                    denoise_fn=self.gm_diffnet,
                    num_timesteps=hparams["f0_timesteps"],
                )
            else:
                self.f0_gen = GaussianMultinomialDiffusion(
                    num_classes=2,
                    denoise_fn=self.gm_diffnet,
                    num_timesteps=hparams["f0_timesteps"],
                )
    ...
    def forward(
        ...
        **kwargs
        ...
        if spk_embed != None:
            ...
            ret["spk_embed"] = spk_embed = self.style_encoder.encode_spk_embed(
                mel_prompt.transpose(1, 2)
            ).transpose(1, 2)

        # style embed
        in_nonpadding = (mel2ph > 0).float()[:, :, None]
        style, vq_loss, indices = self.style_encoder.encode_style(
            target.transpose(1, 2),
            in_nonpadding.transpose(1, 2),
            mel2ph,
            src_nonpadding.transpose(1, 2),
            ph_lengths,
        )
        ret["vq_loss"] = vq_loss
        ...
        decoder_inp = decoder_inp + self.forward_pitch(
            pitch_inp, f0, uv, mel2ph, ret, midi_notes=midi_notes
        )

        # decoder input
        ret["decoder_inp"] = decoder_inp = (
            decoder_inp + spk_embed + style
        ) * tgt_nonpadding
    ...
    def forward_pitch(self, decoder_inp, f0, uv, mel2ph, ret, **kwargs):
        ...
        if hparams["f0_gen"] == "gmdiff":
            f0, uv = self.add_gmdiff_pitch(
                pitch_pred_inp, f0, uv, mel2ph, ret, **kwargs
            )
    ...
    def smooth_f0(self, f0, window_size=5):
        ...
        f0_smooth = F.avg_pool1d(
            f0_padded.unsqueeze(1), kernel_size=window_size, stride=1
        ).squeeze(1)
        return f0_smooth
...
class SADecoder(nn.Module):
    def __init__(self):
        ...
        self.sad = StyleDecoder(
            phone_encoder=None,
            out_dims=80,
            denoise_fn=DIFF_DECODERS[hparams["post_diff_decoder_type"]](hparams),
            timesteps=hparams["post_timesteps"],
            K_step=hparams["post_K_step"],
            loss_type=hparams["post_diff_loss_type"],
            spec_min=hparams["post_spec_min"],
            spec_max=hparams["post_spec_max"],
        )
